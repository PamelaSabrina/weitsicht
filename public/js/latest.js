$(document).ready(function() {

  // Typeform: Call API
    $.getJSON('https://api.typeform.com/v1/form/SfAf6J?key=99151ac80f897f6cd780d9f7fed2ca2d725c3497&completed=true&order_by[]=date_submit,desc&limit=1', function(data) {
    console.log(data);

    var privacy = []

    // Loop over the responses
    for(var i = 0; i < data.responses.length; i++) {

      var category = data.responses[i].answers.dropdown_50447633

      if (category === "Web Language", "Settings", "Future", "Google", "Instagram", "News", "Facebook", "OSX", "Mail", "Spotify") {
        privacy.push(data.responses[i]);

        // Get the keys to each answer within a response
        var keys = Object.keys(data.responses[i].answers);
        var answers = data.responses[i].answers;
        var div = $('<div class="flex-item"></div>');

        // THIS NEEDS AN IMAGE OPTION ADDED IN TYPEFORM AND THE HOOKS EDITED BELOW
        if(answers['dropdown_50447633']) {
            var c = $('<div class="category">'+answers.dropdown_50447633+'</div>')
            div.append(c);
        }

        if(answers['textarea_50046657']) {
            var a = $('<div class="popup-title-latest">'+answers.textarea_50046657+'</div>')
            div.append(a);
        }

        // title future
        if(answers['textfield_50055468']) {
            var a = $('<div class="popup-title-latest">'+answers.textfield_50055468+'</div>')
            div.append(a);
        }

        // answer language
        if(answers['textfield_50045819']) {
            var a = $('<div class="popup-title-latest">'+answers.textfield_50045819+'</div>')
            div.append(a);
        }

        if(answers['textfield_49271969']) {
            var a = $('<div class="author">Author:&nbsp;&nbsp;'+answers.textfield_49271969+'</div>')
            div.append(a);
        }

        // future image
        if(answers['fileupload_50879764']) {
            var c = $("<img class='post-image' src='"+answers.fileupload_50879764+"'>")
            div.append(c);
        }

        if(answers['fileupload_50277123']) {
            var c = $("<img class='post-image-latest' src='"+answers.fileupload_50277123+"'>")
            div.append(c);
        }

        if(answers['textarea_50277200']) {
            var a = $('<div class="popup-answer">'+answers.textarea_50277200+'</div>')
            div.append(a);
        }

        // answer future
        if(answers['textarea_50051241']) {
            var a = $('<div class="popup-answer">'+answers.textarea_50051241+'</div>')
            div.append(a);
        }

        if(answers['fileupload_50277211']) {
            var c = $("<img class='post-image-latest' src='"+answers.fileupload_50277211+"'>")
            div.append(c);
        }

        // answer language

        if(answers['textarea_50051961']) {
            var a = $('<div class="popup-answer-language-latest">'+answers.textarea_50051961+'</div>')
            div.append(a);
        }





      }

      $('#answers').append(div);
    }

    // Opening & closing conent models
    $(".close").click(function(){
      $(this).parents("#modal").hide();
    });

    $(".myBtn").click(function() {
      $(this).next("#modal").show();
    });

    // Scrolling fade function
    $(function() {
      $('body').removeClass('fade-out');
    });

    $(window).scroll(function () {
      var scrollTop = $(window).scrollTop();
      var height = $(window).height();
      $('.logo_container, .slogan, #arrow_container').css({
          'opacity': ((height - scrollTop) / height)
      });
    });
  })
});
